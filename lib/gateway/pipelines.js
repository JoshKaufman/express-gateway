...
module.exports.bootstrap = function ({app, config}) {
  ...
  for (const name in config.gatewayConfig.pipelines) {
    ...
    try {
      ...
      log.error('Failed to register pipeline: ' + name, err);
    }
  }
      ...
      for (const path of mountPaths) {
        ...
        const handler = (req, res, next) => {
          if (!apiEndpointToPipelineMap[route.apiEndpointName]) {
            log.warn('No pipeline configured for endpoint ' + route.apiEndpointName);
            return next(new Error('No pipeline configured for endpoint ' + route.apiEndpointName));
          }
...
function validateGatewayConfig (config) {
  ...
  for (const name in gatewayConfig.pipelines) {
    ...
    // if no policies defined no need to wrap into array
    if (pipeline.policies && !Array.isArray(pipeline.policies)) {
...
function validatePipelinePolicies (policies, config) {
  ...
  if (!policies || !policies.length) {
    throw new Error('pipeline has no policies');
  }
